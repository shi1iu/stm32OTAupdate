...
static const char *custom_prompt = "shi1iu>>  ";   // 默认提示符
...
void shellSetPrompt(Shell *shell, const char *prompt)
{
    custom_prompt = prompt;
}

static void shellDisplayPrompt(Shell *shell)
{
    shellWriteString(shell, custom_prompt);
}
/**
 * @brief shell 任务
 * 
...
void shellTask(void *param)
{
    ...
    char data;
	//shellWriteString(shell, "shell>> ");
		//shellDisplayPrompt(shell);
...
clear, shellClear, clear console);
  ...
  /* USER CODE BEGIN  */
/**
* @brief shell 擦除FLASH的APP区
 */
void shellFlash(void)
{
		Shell *shell = shellGetCurrent();
	  shellPrint(shell, "Shell test");
		//return 0;
	
}
SHELL_EXPORT_CMD(
SHELL_CMD_PERMISSION(0)|SHELL_CMD_TYPE(SHELL_TYPE_CMD_FUNC)|SHELL_CMD_DISABLE_RETURN, 
Flash, shellFlash, 一个测试命令);


/**
* @brief shell 擦除FLASH的APP区
 */
void shellUpdata(void)
{
		Shell *shell = shellGetCurrent();
		g_updata_ready = 1; 
	  shellPrint(shell, "Shell test");
...
SHELL_EXPORT_CMD(
SHELL_CMD_PERMISSION(0)|SHELL_CMD_TYPE(SHELL_TYPE_CMD_FUNC)|SHELL_CMD_DISABLE_RETURN, 
updata, shellUpdata, 用于更新程序使用的命令);
